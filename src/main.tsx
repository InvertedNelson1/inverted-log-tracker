...
const toKg = (fu: string) => (parseFloat(fu || '0') * FU_TO_KG).toFixed(1);
...
const [prAlert, setPrAlert] = useState<any>(null);
...
const allEntries = useMemo(() => workouts.flatMap((w: any) => w.entries.map((e: any) => ({ ...e, date: w.date, workoutId: w.id }))), [workouts]);

const getPRs = useMemo(() => {
const prs: Record<string, number> = {};
allEntries.forEach((e: any) => {
const max = Math.max(...e.sets.map((s: any) => parseFloat(s.weight) || 0));
...
const newMax = Math.max(...entry.sets.map((s: any) => parseFloat(s.weight) || 0));
let updated: any[];
const existingIdx = workouts.findIndex((w: any) => w.date === today);
if (existingIdx > -1) {
updated = [...workouts];
...
{activeTab === 'LOG' && (
<div>
<LogTab onSave={addEntry} prs={getPRs} todayWorkout={workouts.find((w: any) => w.date === new Date().toISOString().split('T')[0])} />
...
const LogTab = ({ onSave, prs, todayWorkout }: any) => {
...
const handleRepsChange = (i: number, val: string) => {
const n = [...sets] as any[];
n[i].reps = val;
setSets(n);
};
...
const handleTriple = () => {
const n = [...sets] as any[];
n[0].reps = (parseInt(n[0].reps || '0') * 3).toString();
setSets(n);
};

const removeSet = (i: number) => setSets((sets as any[]).filter((_: any, idx: number) => idx !== i));

return (
<div>
<h2 style={{ fontSize: '2rem', marginBottom: '20px' }}>LOG SESSION</h2>
<select value={ex} onChange={(e: any) => setEx(e.target.value)} style={{ width: '100%', marginBottom: '10px' }}>
...
<button onClick={handleTriple} style={{ flex: 1, background: '#222', color: '#fff' }}>x3</button>
...
<input placeholder="REPS" value={s.reps} onChange={(e: any) => handleRepsChange(i, e.target.value)} style={{ width: '60px' }} />
<div style={{ flex: 1 }}>
<input placeholder="WEIGHT" value={s.weight} onChange={(e: any) => handleWeightChange(i, e.target.value)} style={{ width: '100%' }} />
<div style={{ fontSize: '0.7rem', color: '#555' }}>{toKg(s.weight)} KG</div>
</div>
<button onClick={() => removeSet(i)} style={{ color: '#822', background: 'none' }}>X</button>
</div>
))}

<textarea placeholder="Note..." value={note} onChange={(e: any) => setNote(e.target.value)} style={{ width: '100%', height: '60px', marginTop: '10px' }} />
...
const HistTab = ({ workouts, prs, setWorkouts }: any) => (
<div>
<h2 style={{ fontSize: '2rem', marginBottom: '20px' }}>HISTORY</h2>
{[...workouts].sort((a: any, b: any) => new Date(b.date).getTime() - new Date(a.date).getTime()).map((w: any) => (
...
<h3>{w.date}</h3>
<button onClick={() => setWorkouts(workouts.filter((x: any) => x.id !== w.id))} style={{ color: '#522', background: 'none' }}>DELETE</button>
</div>
{w.entries.map((e: any, i: number) => (
<div key={i} style={{ marginBottom: '10px' }}>
<div style={{ fontWeight: 'bold' }}>{e.exercise}</div>
{e.sets.map((s: any, si: number) => (
<div key={si} style={{ color: '#888', fontSize: '0.9rem' }}>{s.reps} x {s.weight} FU ({toKg(s.weight)} KG) - {((parseFloat(s.weight) / (prs[e.exercise] || 1)) * 100).toFixed(0)}% PR</div>
))}
</div>
))}
</div>
...
<div style={{ color: '#888' }}>{prs[ex]} FU / {toKg(prs[ex].toString())} KG</div>
</div>
</div>
))}
</div>
