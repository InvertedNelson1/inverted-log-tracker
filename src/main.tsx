...
const App = () => {
        ...
        // Note: no generic type on useMemo to avoid TSX parsing issues with <Type>
        const allEntries: EntryWithMeta[] = useMemo(
                  () => workouts.flatMap(w => w.entries.map(e => ({ ...e, date: w.date, workoutId: w.id }))),
                  [workouts]
                );

        const getPRs: PRMap = useMemo(() => {
                  const prs: PRMap = {};
        ...
        return (
                  <div>
                        ...
                        <div style={{ display: 'flex', gap: '5px', marginBottom: '20px' }}>
                                ...
                                <button onClick={handleTriple} style={{ flex: 1, background: '#222', color: '#fff' }}>x3</button>button>
                        ...
                        {sets.map((s, i) => (
                                <div key={i} style={{ display: 'flex', gap: '10px', alignItems: 'center', marginBottom: '10px' }}>
                                          ...
                                          <input placeholder="REPS" value={s.reps} onChange={e => handleRepsChange(i, e.target.value)} style={{ width: '60px' }} />
                                          <div style={{ flex: 1 }}>
                                                      <input placeholder="WEIGHT" value={s.weight} onChange={e => handleWeightChange(i, e.target.value)} style={{ width: '100%' }} />
                                                      <div style={{ fontSize: '0.7rem', color: '#555' }}>{toKg(s.weight)} KG</div>div>
                                          </div>div>
                                          <button onClick={() => removeSet(i)} style={{ color: '#822', background: 'none' }}>X</button>button>
                                </div>div>
                              ))}
                        ...
                        {sorted.map(w => (
                                <div key={w.id} style={{ marginBottom: '20px', border: '1px solid #222', padding: '15px' }}>
                                          <div style={{ display: 'flex', justifyContent: 'space-between', borderBottom: '1px solid #222', marginBottom: '10px' }}>
                                                      <h3>{w.date}</h3>h3>
                                                      <button onClick={() => setWorkouts(workouts.filter(x => x.id !== w.id))} style={{ color: '#522', background: 'none' }}>DELETE</button>button>
                                          </div>div>
                                      {w.entries.map((e, i) => (
                                                  <div key={i} style={{ marginBottom: '10px' }}>
                                                                <div style={{ fontWeight: 'bold' }}>{e.exercise}</div>div>
                                                        {e.sets.map((s, si) => (
                                                                        <div key={si} style={{ color: '#888', fontSize: '0.9rem' }}>
                                                                              {s.reps} x {s.weight} FU ({toKg(s.weight)} KG) - {((parseFloat(s.weight) / (prs[e.exercise] || 1)) * 100).toFixed(0)}% PR
                                                                        </div>div>
                                                                      ))}
                                                  </div>div>
                                                ))}
